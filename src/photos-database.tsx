...
import {commentItem, photoDatabaseItem} from "./types/common.type";

export function saveToLocalStorage (id:number, data:{comments:commentItem[],like: boolean}) {
    localStorage.setItem(id.toString(), JSON.stringify(data));
}

export const photosDatabase = (function pushComments () {
    let database = [
        {
    ...
    return database.map(item=> {
        const data = localStorage.getItem(item.id.toString());
        if (data) {
            const {like, comments} = JSON.parse(data);
            item.comments = comments as commentItem[];
            item.like = like as boolean;
        }
        return item
    });
})()

export const photosDatabase2: photoDatabaseItem[] = [
    {
        src: "/images/nature/05da8387a34b4b8dc4896287fe722451.jpg",
        id: 1,
        alt: 'nature',
